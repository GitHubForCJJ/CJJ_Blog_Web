//-----------------------------------------------------------------------------------
// <copyright file="HttpHelper.cs" company="EastWestWalk Enterprises">
// * copyright: (C) 2018 东走西走科技有限公司 版权所有。
// * version  : 1.0.0.0
// * author   : Administrator
// * fileName : HttpHelper.cs
// * history  : created by Administrator 2018-06-26 上午 09:42:38
// </copyright>
// <summary>
//   FastDev.Http.HttpHelper
// </summary>
//-----------------------------------------------------------------------------------
...
namespace CJJ.WeChart.API.Helper
{
    ...
    public class HttpHelper
    {
            ...
            if (item.Header != null && item.Header.Count > 0)
            {
                foreach (string key in item.Header.AllKeys)
                {
                    switch (key.ToLower())
                    {
                        case "host":
                            {
                                request.Host = item.Header[key];
                                break;
                            }
                        case "connection":
                            {
                                request.Connection = item.Header[key];
                                break;
                            }
                        case "content-length":
                            {
                                request.ContentLength = Convert.ToInt64(item.Header[key]);
                                break;
                            }
                        case "accept":
                            {
                                request.Accept = item.Header[key];
                                break;
                            }
                        case "user-agent":
                            {
                                request.UserAgent = item.Header[key];
                                break;
                            }
                        case "content-type":
                            {
                                request.ContentType = item.Header[key];
                                break;
                            }
                        case "referer":
                            {
                                request.Referer = item.Header[key];
                                break;
                            }

                        case "cookie":
                            {
                                var ary = item.Header[key].Split(';');
                                var cookies = new CookieContainer();
                                foreach (var im in ary)
                                {
                                    var c = im.Split('=');
                                    if (c.Length == 2)
                                    {
                                        cookies.Add(new Cookie() { Name = c[0].Trim(), Value = System.Web.HttpUtility.UrlEncode(c[1].Trim()), Domain = new Uri(item.URL).Host });
                                    }
                                }
        ...
        /// <returns></returns>
        public string UrlEncode(string url, Encoding encoding)
        {
